...
EANT_TEST_GENTOO_CLASSPATH="lucene-${SLOT},lucene-queries-${SLOT},randomized-runner,spatial4j"
...
src_prepare() {
	...
	eapply "${FILESDIR}/${P}-use-system-lucene-queries.patch"
	if use test; then
...
src_install() {
	java-pkg_newjar build/spatial/*.jar
	use source && java-pkg_dosrc spatial/src/java/*
}
